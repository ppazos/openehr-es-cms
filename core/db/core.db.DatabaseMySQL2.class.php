...
class DatabaseMySQL2 {
   ...
   public function connect( $dbhost, $dbuser, $dbpass, $dbName )
   {
      ...
      Logger::getInstance()->log("DatabaseMySQL::connect ");

      if ( !$this->connection )
      {
         throw new Exception( "No pudo conectarse a MySQL: " . mysqli_error() );
      }
   ...
   public function disconnect ()
   {
      ...
      {
         mysqli_close($this->connection); // No necesito pasar la coneccion
   ...
   public function query( $query )
   {
      ...
      if (!$result = mysqli_query($this->connection, $query))
         throw new Exception('La consulta fall&oacute;: ' . mysqli_error($this->connection));
...
//   }
   ...
   private function evaluateReferenceValue( $refVal )
   {
      ...
      if ( $refVal === NULL ) return 'NULL'; // Ver si se sigue necesitando por la correccion de IS NULL e IS NOT NULL
